    ...
    def action_genrate_new_variant(self):
        ...
        if self.default_code:
            if "-" in self.default_code:
                ...
                product_variant_id = self.env["product.variant"].search(
                    [("name", "=", part_char)], limit=1
                )
                ...
                if product_variant_id:
                    ...
                    raise UserError(
                        "{0} is not a variant level that is currently"
                        " maintained in the PLM app for Product"
                        " Variants.".format(part_char)
                    )

            else:
                raise UserError(
                    "No valid current variant could be determined from"
                    " this product's Part Number. A new variant cannot"
                    " be generated using this method."
                )
        ...
        if variant_level_list:
            ...
            if index + 1 < len(variant_level_list):
                ...
                seq_product_variant = self.env["product.variant"].search(
                    [("sequence", "=", new_genrated_sequence)], limit=1
                )
                if seq_product_variant:
                    final_char = seq_product_variant.name
                    # final_part_number = self.default_code.split('-')[0]
                    # +"-"+final_char
                    ...
                    default_production_state = self.env[
                        "production.state"
                ...
                raise UserError(
                    "No subsequent Product Variant has been defined in the"
                    " PLM app. It must be defined if you wish to generate"
                    " a new Product Variant using this capability."
                )

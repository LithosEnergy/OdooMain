...
class DocumentWizard(models.Model):
    ...
    def _get_item_numbers(self):
        for rec in self:
            ...
            if first_line_rec.product_revisions_id:
                ...
                for value, index in enumerate(
                    first_line_rec.mrp_eco_product_id.document_wizard_line,
                    start=1,
                ):
                    index.update({"item_number": value})
    ...
    mrp_eco_product_id = fields.Many2one(
        "mrp.eco.product", string="Mrp Eco Product Id"
    )
    full_url = fields.Char(string="URL", compute="_compute_full_url")
    product_revisions_id = fields.Many2one(
        "product.revisions", string="Product Revisions Id"
    )

    def create(self, vals_list):
        active_id = (
            self.env["mrp.eco.product"]
        ...
        for val in vals_list:
            if "bom_document_wizard_test" in val:  # for type bom
                ...
                if active_id:
                    mrp_eco_product_lst = (
                        self.env["mrp.eco.product"].search([]).ids
                    )
                    if active_id in mrp_eco_product_lst:
                        val["mrp_eco_product_id"] = active_id

        lines = super(DocumentWizard, self).create(vals_list)
        return lines
    ...
    def _compute_full_url(self):
        ...
        for record in self:
            # record.full_url = "%s/web/image?model=document.wizard&id=%s&
            # field=upload_document" % (base_url, record.id)
            record.full_url = (
                "%s/web/image/document.wizard/%s/upload_document/%s"
                % (base_url, record.id, record.upload_document_name)
            )

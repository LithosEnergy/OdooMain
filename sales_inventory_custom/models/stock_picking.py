...
class StockPicking(models.Model):
	...
	approval_temp_id = fields.Many2one('stock.transfer.approval.temp',string="Approval Template")
	...
	approval_history_ids = fields.One2many('stock.approval.history', 'picking_history_id', string="Approval History")
	...
	def _compute_valid_for_approval(self):
		...
		for obj in self:
			if obj.submit_approval and not obj.fully_approved:
				find_line_not_approved = obj.approval_line_ids.filtered(lambda x:self.env.uid  in x.req_user_ids.ids and x.status in ('submitted','reject'))
	...
	def _compute_valid_for_reject(self):
		...
		for obj in self:
			if obj.submit_approval and not obj.fully_approved:
				find_line_not_reject = obj.approval_line_ids.filtered(lambda x:self.env.uid  in x.req_user_ids.ids and x.status in ('submitted','approved'))
			...
			if find_line_not_reject:
				for line in find_line_not_reject:
					...
					vals = {
						'name':line.name,
						'status':'reject',
						'approval_stage_id':self.picking_type_id.type_approval_id.id,
	...
	def _compute_fully_approved(self):
		...
		for obj in self:
			if obj.approval_line_ids:
				fully_approved = all(line.status == 'approved' for line in obj.approval_line_ids)
	...
	def onchange_picking_type_approval(self):
		for obj in self:
			approval_list = []
			if not obj.picking_type_id or not obj.picking_type_id.type_approval_id:
			...
			if obj.picking_type_id and obj.picking_type_id.type_approval_id:
				...
				for item in obj.picking_type_id.type_approval_id.approval_item_ids:
					vals = {
						'name':item.name,
						'status':'not_yet',
						'approval_stage_id':obj.picking_type_id.type_approval_id.id,
						'picking_approve_id': obj.id,
					}
					if item.user_ids:
						vals.update({'req_user_ids':[(6,0,item.user_ids.ids)]})
	...
	def button_validate(self):
		if self.approval_line_ids:
			not_approved = self.approval_line_ids.filtered(lambda x:x.status != 'approved')
			if not_approved:
				raise UserError(_('This stock move can not be performed until all approvals have been performed'))
	...
	def button_submit_approval(self):
		...
		email_values = {}
		template_id = self.env['ir.model.data'].xmlid_to_res_id('sales_inventory_custom.stock_approval_request_mail_template', raise_if_not_found=False)
			...
			for line in self.approval_line_ids:
				...
				for user in line.req_user_ids:
					if user.partner_id and user.partner_id.email and template:
						email_values = {'email_to':user.partner_id.email,'url_id':url_id}
						template[0].with_context(email_values).send_mail(self.id, force_send=True)
...
class StockPickingType(models.Model):
	...
	type_approval_id = fields.Many2one('stock.transfer.approval.temp',string="Approval Template")
...
class ReportBomStructure(models.AbstractModel):
    ...
    def get_product_forecasted(self, product_id, warehouse_id):
        ...
        if warehouse_id:
            picking_type_ids = (
                ...
                .sudo()
                .search_read([("warehouse_id", "=", warehouse_id)], ["id"])
            )
    ...
    def get_product_pre_inspection(self, product_id, warehouse_id):
        ...
        if warehouse_id:
            warehouse_id = (
                self.env["stock.warehouse"].sudo().browse(warehouse_id)
            )
    ...
    def _get_bom(
        ...
        operations = self._get_operation_line(
            ...
            bom,
            float_round(
                bom_quantity, precision_rounding=1, rounding_method="UP"
            ),
            0,
        )
        ...
        if "Warehouse" in self.env.context and self.env.context["Warehouse"]:
            if int(self.env.context["Warehouse"]):
                qty_available = self.get_product_warehouse_stock(
                    product.id, int(self.env.context["Warehouse"])
                )
                qty_pre_available = self.get_product_pre_inspection(
                    product.id, int(self.env.context["Warehouse"])
                )
                qty_forecasted = self.get_product_forecasted(
                    product.id, int(self.env.context["Warehouse"])
                )
            else:
    ...
    def _get_bom_lines(self, bom, bom_quantity, product, line_id, level):
        ...
        for component in components:
            component_list.update({component["prod_id"]: count_list})
            count_list += 1
        ...
        for line in bom.bom_line_ids:
            ...
            if (
                "Warehouse" in self.env.context
                and self.env.context["Warehouse"]
                and int(self.env.context["Warehouse"])
            ):
                qty_available = self.get_product_warehouse_stock(
                    line.product_id.id, int(self.env.context["Warehouse"])
                )
                qty_pre_available = self.get_product_pre_inspection(
                    line.product_id.id, int(self.env.context["Warehouse"])
                )
            ...
            total += sub_total
            if line.product_id.id in list(component_list.keys()):
                find_index = component_list[line.product_id.id]
                d1 = components[find_index]
                d3 = {}
                d3.update(d1)
                d3.update(vals)
                final_components.append(d3)
            else:
                final_components.append(vals)
    ...
    def get_html(self, bom_id=False, searchQty=1, searchVariant=False):
        ...
        for warehouse in warehouse_ids:
            warehouse_data.append({"id": warehouse.id, "name": warehouse.name})

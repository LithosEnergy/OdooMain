...
class Generate_Product_Wizard(models.TransientModel):
    ...
    def _get_default_category_id(self):
        ...
        if category:
            ...
            err_msg = _(
                ...
                " category in order to be able to create products."
            )
    ...
    route_ids = fields.Many2many(
        ...
        "stock_route_generate_product",
        "generate_product_id",
        "route_id",
        "Routes",
        domain=[("product_selectable", "=", True)],
        help="Depending on the modules installed,"
             " this will allow you to define the route of"
             " the product: whether it will be bought,"
    ...
    vendor = fields.Many2one(
        "res.partner", string="Vendor", domain="[('supplier_rank', '>', 0)]"
    )
    ...
    def create_product(self):
        production_state = self.env["production.state"].search(
            [("default", "=", "True")]
        )
        if not production_state:
            raise UserError(
                _(
                    "A default Production State must be defined in"
                    " PLM Configuration prior to using this wizard"
        ...
        product.version = (
            0
        )  # product.template.version set 0 because default its 1

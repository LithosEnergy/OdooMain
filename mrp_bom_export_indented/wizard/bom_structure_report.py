...
class BomStructureReportWizard(models.TransientModel):
    ...
    def print_bom_children(self, ch, sheet1, row, level, col2, col3, qty=1.0, uom=False):
        style1 = XFStyle()
        al = Alignment()
        ...
        if ch.product_id and ch.product_id.default_code and len(ch.product_id.default_code) > col2:
            col2 = len(ch.product_id.default_code)
        if ch.product_id and ch.product_id.name and len(ch.product_id.name) > col3:
        ...
        for product_bom in ch.product_id.bom_ids:
            if product_bom.routing_id:
                routing_id = product_bom.routing_id.name
            if product_bom.type:
                bom_type = dict(product_bom._fields['type'].selection).get(product_bom.type) or ''
        sheet1.write(i, 2, str(ch.product_id.default_code or ''), style_center)
        sheet1.write(i, 3, str(ch.product_id.display_name or ''))
        sheet1.write(i, 4, str(dict(ch.product_id.product_tmpl_id._fields['type'].selection).get(ch.product_id.product_tmpl_id.type) or ''))
        ...
        sheet1.write(i, 12, str(routing_id), style_center)
        if ch.product_id.seller_ids:
            for seller in ch.product_id.seller_ids:
                # sheet1.write(i, 13, '>',style_center)
                sheet1.write(i, 13, str(seller.name.name or ''), style_center)
                sheet1.write(i, 14, str(seller.product_code or ''), style_center)
        ...
        if ch.child_bom_id and ch.child_bom_id.bom_line_ids.ids:
            ...
            for child in ch.child_line_ids:
                i = self.print_bom_children(child, sheet1, i, j, col2, col3, pqty, puom)
        j -= 1
        return i
    ...
    def get_print_report(self):
        ...
        for o in bom_ids:
            ...
            if o.product_id and o.product_id.default_code and len(o.product_id.default_code) > col2:
                col2 = len(o.product_id.default_code)
            if o.product_id and o.product_id.name and len(o.product_id.name) > col3:
            ...
            sheet1.write(i, 2, str(o.product_tmpl_id.default_code or ''), style_center)
            sheet1.write(i, 3, str(o.product_tmpl_id.name or ''), style1)
            sheet1.write(i, 4, str(dict(o.product_tmpl_id._fields['type'].selection).get(o.product_tmpl_id.type) or ''), style1)# ********************************
            sheet1.write(i, 5, route_ids or '')
            sheet1.write(i, 6, str(price), style_center)
            sheet1.write(i, 7, str(o.product_tmpl_id.weight or 0.0), style_center)
            ...
            sheet1.write(i, 11, str(dict(o._fields['type'].selection).get(o.type) or ''), style_center) #***************************
            sheet1.write(i, 12, str(o.routing_id.name or ''), style_center)

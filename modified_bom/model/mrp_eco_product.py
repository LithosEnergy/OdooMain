...
class InheritMRPEcoProduct(models.Model):
    _inherit = "mrp.eco.product"

    new_partline_bom_id = fields.Many2one(
        "mrp.bom", "New Bill of Materials", copy=False
    )
    new_partline_bom_revision = fields.Integer(
        related="new_partline_bom_id.version", store=True, readonly=False
    )
    is_ecotype_bom = fields.Boolean(string="Is ECO Type BOM", default=False)
    is_bomid_for_current_line = fields.Boolean(
        string="Is BOM id available for cuurent line", default=False
    )

    # this below field is used for button
    # action_new_revision_for_partline to handle autorun from js
    action_new_revision_depend = fields.Boolean(
        string="action_new_revision_depend", default=True
    )
        ...
        if self:
            if self.new_partline_bom_id:
                ...
                return {
                    ...
                    "context": dict(
                        default_product_tmpl_id=self.affected_product_id.id,
                        default_product_id=self.affected_product_id.product_variant_id.id,
                    ),
                }
            ...
            if not self.action_new_revision_depend:
                if self.affected_product_id:
                    if not self.affected_product_id.bom_ids:
                        ...
                        bom_create = self.affected_product_id.bom_ids.create(
                            vals
                        )
                        ...
                        maxVerionItem = max(
                            bom_list, key=lambda x: x["version"]
                        )
